	...
	list_for_each_entry(ch, list, ch_list) {
		...
		if (tmp != ch->last_state) {
			...
			if (zte_smd_wakeup &&
				...
				pr_info("smd_wakeup: %s wakeup app, SMD ch%d\n", subsys, ch->n);
				pr_info("smd_wakeup:'%s' state change %d->%d\n", ch->name, ch->last_state, tmp);
		...
		if (ch_flags & 0x3) {
			...
			if (zte_smd_wakeup &&
				...
				pr_info("smd_wakeup:%s wakeup app,ch %d '%s' Data event\n", subsys, ch->n, ch->name);
				if (is_qmi_channel(ch->name))
		...
		if (ch_flags & 0x4 && !state_change) {
			...
			if (zte_smd_wakeup &&
				...
				pr_info("smd_wakeup:%s wakeup app,SMD ch %d '%s' State update\n",
								subsys, ch->n, ch->name);
				if (ch->type != SMD_APPS_RPM)
					not_rpm_smd = 1;
				else
