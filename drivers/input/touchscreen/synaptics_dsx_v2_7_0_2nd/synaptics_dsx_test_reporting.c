...
static int synaptics_rawdata_test_read(struct seq_file *m, void *v)
{
	...
	coordinate_test_fail = kzalloc(num_of_rx * num_of_tx * sizeof(short) * 2, GFP_KERNEL);
...
static int ts_short_circuit_test_val(struct seq_file *m, void *v)
{
	int test_result = 0;
	int num_of_tx = f54->tx_assigned;
	int num_of_rx = f54->rx_assigned;
	...
	SYNA_INFO("%s: prj is P996A20,return it.\n", __func__);
	test_result = 0;
	goto exit;
#endif
...
exit:
	...
	seq_printf(m, "%d\n", (test_result == 0) ? 1 : SHORT_CIRCUIT_FAIL);

	return 0;
}

static int synaptics_short_circuit_test_open(struct inode *inode, struct file *file)
{
	return single_open(file, ts_short_circuit_test_val, NULL);
}

static const struct file_operations short_circuit_test_proc_ops = {
	.owner	= THIS_MODULE,
	.open	= synaptics_short_circuit_test_open,
	.read	= seq_read,
	.llseek	= seq_lseek,
	.release	= single_release,
};
