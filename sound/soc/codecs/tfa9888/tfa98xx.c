...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	bprofile = kcalloc(nprof, sizeof(struct tfa98xx_baseprofile *), GFP_KERNEL);
	...
	for (prof = 0; prof < nprof; prof++) {
		...
		bprofile[prof] = devm_kzalloc(tfa98xx->codec->dev, sizeof(**bprofile), GFP_KERNEL);
		if (!bprofile[prof])
			goto err_release_prev_bprofile;

		bprofile[prof]->len = 0;
		bprofile[prof]->item_id = -1;
		INIT_LIST_HEAD(&bprofile[prof]->list);

		/* copy profile name into basename until the . */
		get_profile_basename(bprofile[prof]->basename, tfaContProfileName(tfa98xx->handle, prof));
		bprofile[prof]->len = strlen(bprofile[prof]->basename);
		 ...
		 */
		if (is_profile_in_list(bprofile[prof]->basename, bprofile[prof]->len) == 0 &&
			 is_calibration_profile(tfaContProfileName(tfa98xx->handle, prof)) == 0) {
			/* the profile is not present, add it to the list */
			list_add(&bprofile[prof]->list, &profile_list);
			bprofile[prof]->item_id = id++;

			pr_debug("profile added [%d]: %s\n", bprofile[prof]->item_id, bprofile[prof]->basename);

			if (tfacont_get_max_vstep(tfa98xx->handle, prof)) {
				name = devm_kzalloc(tfa98xx->codec->dev, MAX_CONTROL_NAME, GFP_KERNEL);
				if (!name) {
					list_del(&bprofile[prof]->list);
					devm_kfree(tfa98xx->codec->dev, bprofile[prof]);
					goto err_release_prev_bprofile;
				}

				scnprintf(name, MAX_CONTROL_NAME, "%s %s Playback Volume",
				tfa98xx->fw.name, bprofile[prof]->basename);
		 ...
		 */
		add_sr_to_profile(tfa98xx, bprofile[prof]->basename, bprofile[prof]->len, prof);
	}
...
err_release_prev_bprofile:
	for (prof--; prof >= 0; prof--) {
		list_del(&bprofile[prof]->list);
		devm_kfree(tfa98xx->codec->dev, bprofile[prof]);
	}
	for (mix_index--; mix_index >= 0; mix_index--) {
		name = (char *)tfa98xx_controls[mix_index].name;
		devm_kfree(tfa98xx->codec->dev, name);
	}
	kfree(name);
err_release_tfa98xx_controls:
	devm_kfree(tfa98xx->codec->dev, tfa98xx_controls);
err_release_bprofile_pointer:
	kfree(bprofile);
	return -ENOMEM;
}
